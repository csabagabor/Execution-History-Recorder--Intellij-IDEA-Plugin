...
import reuse.sequence.diagram.Info;
import reuse.sequence.generator.filters.ImplementClassFilter;
...
public class SequenceGenerator extends JavaElementVisitor {
    ...
    public void visitMethod(PsiMethod psiMethod) {
        MethodDescription method = createMethod(psiMethod);
        if (makeStackFrameExceptCurrentStackIsRecursive(method)) return;
    ...
    private void findAbstractImplFilter(PsiCallExpression callExpression, PsiMethod psiMethod) {
        try {
            ...
            if (PsiUtil.isAbstract(containingClass)) {
                ...
                PsiMethodCallExpression psiStackFrameExpression = (PsiMethodCallExpression) callExpression;
                PsiExpression qualifierExpression = psiStackFrameExpression.getMethodExpression().getQualifierExpression();
    ...
    private void stackFrame(PsiMethod psiMethod) {
        ...
        if (depth < params.getMaxDepth() - 1) {
            ...
            currentStack.stackFrame(createMethod(psiMethod));
    }
    ...
    private boolean makeStackFrameExceptCurrentStackIsRecursive(MethodDescription method) {
        if (topStack == null) {
            ...
            if (params.isNotAllowRecursion() && currentStack.isRecursive(method))
                return true;
            currentStack = currentStack.stackFrame(method);
        }
        return false;
    }
