...
public class DisplayLink extends ScreenObject {
    ...
    void drawText(Graphics2D g2) {
        Paint textColor = TEXT_COLOR;
        try {
            if (_from != null && _from.getObjectInfo().hasAttribute("highlight:true")) {
                textColor = JBColor.RED;
            } else if (_to != null && _to.getObjectInfo().hasAttribute("highlight:true")) {
                textColor = JBColor.RED;
            }
        } catch (Exception e) {
        }
    ...
    void drawLine(Graphics2D g2) {
        Paint linkColor = LINK_COLOR;
        try {
            if (_from != null && _from.getObjectInfo().hasAttribute("highlight:true")) {
                linkColor = JBColor.RED;
            } else if (_to != null && _to.getObjectInfo().hasAttribute("highlight:true")) {
                linkColor = JBColor.RED;
            }
        } catch (Exception e) {
